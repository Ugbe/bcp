"""Conservative one-stage multitask LoRA training for the Qwen3.5 family.
...
The same recipe serves every dense Qwen3.5 size. Pair each base checkpoint with
the corpus variant that teaches the matching public name (see
``derive_identity_variant.py``) and pass that name as ``--identity-name`` so the
trainer refuses a mismatched corpus before loading the model.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--identity-name",
        default="Atom Electron 1-9B",
        help="Public model name the corpus teaches; every system prompt must contain it.",
    )
    parser.add_argument(
...
# {args.identity_name} multitask LoRA
...
def check_identity(
    train_records: list[dict], validation_records: list[dict], identity_name: str
) -> None:
    """Refuse a corpus whose system prompts teach a different public name.

    Each base checkpoint size has its own corpus variant; training Qwen3.5-4B
    on rows that say "Atom Electron 1-9B" would be a silent identity error.
    """
    for split, records in (("train", train_records), ("validation", validation_records)):
        for number, record in enumerate(records, start=1):
            ...
            if not isinstance(system, str) or identity_name not in system:
                raise ValueError(
                    f"{split} row {number} (query {record.get('query_id')}) does not "
                    f"carry the identity name {identity_name!r} in its system prompt"
                )
...
def prompt_completion_records(records: list[dict]) -> list[dict]:
    """Represent each view as context plus one supervised assistant target.

    Unsloth currently rejects TRL's assistant_only_loss path for Qwen3.5
    because the checkpoint is classified as vision-language. Conversational
    prompt/completion data uses TRL's supported completion mask instead: all
    preceding system, user, assistant, and tool messages are context-only, and
    the final assistant message is the sole supervised completion for that
    deliberately constructed dataset view.
    """
    output = []
    for record in records:
        ...
        if len(messages) < 3 or messages[-1].get("role") != "assistant":
            raise ValueError(
                f"Query {record.get('query_id')} must end with an assistant target"
            )
        output.append(
            {
                "prompt": copy.deepcopy(messages[:-1]),
                "completion": [copy.deepcopy(messages[-1])],
                "tools": copy.deepcopy(record.get("tools") or []),
...
def token_length(processor, record: dict) -> int:
    ...
    # Transformers may return a plain list, a dict, or a BatchEncoding here.
...
def main() -> None:
    ...
    check_identity(train_records, validation_records, args.identity_name)
    ...
    train_dataset = Dataset.from_list(prompt_completion_records(processor_train_records))
    validation_dataset = Dataset.from_list(
        prompt_completion_records(processor_validation_records)
    )
    ...
    training_config = SFTConfig(
        ...
        warmup_steps=max(1, math.ceil(estimated_optimizer_steps * 0.1)),
        ...
        fp16=False,
        logging_steps=1,
        # Evaluate explicitly only after the trained adapter has been saved.
        # Epoch-end VLM evaluation can materialize batch-sized vocabulary logits
        # before Trainer reaches its save hook.
        eval_strategy="no",
    ...
    print(f"Saved LoRA adapter to {adapter_dir} before evaluation")

    # Release cached training allocations before the longest validation rows.
