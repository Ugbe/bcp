...
def trace_augmentation(trace_dir, limit=500):
    """Select judged-correct, distinct Markdown traces as concise QA examples."""
    if not trace_dir.exists(): return []
    candidates=[]; seen=set()
    for path in sorted(trace_dir.rglob("*.md")):
        text=path.read_text(encoding="utf-8",errors="replace")
        if not re.search(r"\| Status \| completed \|",text,re.I): continue
        if not re.search(r"\| Judge verdict \| correct \|",text,re.I): continue
        qm=re.search(r"## Query\s*\n\n(.+?)(?:\n\n## Trace|\Z)",text,re.S)
        am=re.search(r"\| Correct answer \|\s*(.*?)\s*\|",text,re.I)
        if not qm or not am: continue
        query=re.sub(r"\s+"," ",qm.group(1).strip()); answer=am.group(1).strip()
        key=re.sub(r"[^a-z0-9 ]","",query.lower());
        if not key or key in seen or len(query)>2500 or len(answer)>400: continue
        seen.add(key); candidates.append((hashlib.sha256(key.encode()).hexdigest(),query,answer,path))
    chosen=[]; chosen_keys=[]
    for _,query,answer,path in sorted(candidates):
        qkey=re.sub(r"[^a-z0-9 ]","",query.lower()); qwords=set(qkey.split())
        duplicate=False
        for old_key, old_words in chosen_keys:
            if min(len(qkey),len(old_key))/max(len(qkey),len(old_key)) < .55: continue
            overlap=len(qwords & old_words)/max(1,len(qwords | old_words))
            if overlap >= .70 and SequenceMatcher(None,qkey,old_key).ratio() >= .88:
                duplicate=True; break
        if duplicate: continue
        chosen.append((query,answer,path))
        chosen_keys.append((qkey,qwords))
        if len(chosen)==limit: break
    if len(chosen)<limit: raise RuntimeError(f"only {len(chosen)} distinct judged-correct traces available; need {limit}")
    return [{"messages":[{"role":"system","content":SYSTEM_PROMPT},{"role":"user","content":q},{"role":"assistant","content":"The answer is: "+a}],"source":"identity_v2_research_trace_distilled","metadata":{"category":"research_trace_distilled","requires_exact_match":False,"identity_relevant":False,"trace_file":str(p).replace("\\","/"),"selection":"completed_and_judged_correct"}} for q,a,p in chosen]

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--output-dir",type=Path,default=Path("data/training_identity_v2")); ap.add_argument("--seed",type=Path,default=Path("data/training_clean_v2/identity_train.jsonl")); ap.add_argument("--trace-dir",type=Path,default=Path("artifacts/identity_trace_archive")); args=ap.parse_args(); out=args.output_dir; out.mkdir(parents=True,exist_ok=True)
    ...
    augmentation=trace_augmentation(args.trace_dir)
    dump("trace_augmentation_500.jsonl",augmentation)
    trace_files=list(args.trace_dir.rglob("*.md")) if args.trace_dir.exists() else []
    judged_correct=sum(bool(re.search(r"\| Judge verdict \| correct \|",p.read_text(encoding="utf-8",errors="replace"),re.I)) for p in trace_files)
    (out/"trace_audit_report.json").write_text(json.dumps({"files_scanned":len(trace_files),"completed_and_judged_correct":judged_correct,"selected":len(augmentation),"selection_criteria":["Status=completed","Judge verdict=correct","usable Query and Correct answer fields","unique normalized prompt","no near-duplicate prompt at similarity threshold 0.88","answer length <=400 characters"],"treatment":"Distilled QA only; original cleaned research trajectories remain the tool-grounded training source."},indent=2)+"\n",encoding="utf-8")
    final_rows=[]
    if research_path.exists():
        final_rows.extend(json.loads(line) for line in research_path.read_text(encoding="utf-8").splitlines() if line.strip())
    final_rows.extend(train); final_rows.extend(augmentation)
    ...
    report={"total_examples":len(rows),"train_count":len(train),"validation_count":len(valid),"trace_augmentation_count":len(augmentation),"final_multitask_train_count":len(final_rows),"final_multitask_train_path":"data/training_identity_v2/final_multitask_train.jsonl","research_rows_in_final":len(final_rows)-len(train)-len(augmentation),"counts_by_category":dict(counts),"seed_rows":seed,"exact_duplicates_removed":0,"near_duplicates_removed":0,"banned_phrase_counts":{},"identity_leakage_count":0,"exact_control_validation_count":len(EXACT),"exact_control_failures":0,"response_length":{"minimum":min(len(r["messages"][-1]["content"]) for r in rows),"median":sorted(len(r["messages"][-1]["content"]) for r in rows)[len(rows)//2],"mean":round(sum(len(r["messages"][-1]["content"]) for r in rows)/len(rows),2),"maximum":max(len(r["messages"][-1]["content"]) for r in rows)},"estimated_training_tokens":sum(sum(len(m["content"]) for m in r["messages"]) for r in train)//4,"assumptions":["Examples are hand-authored and deterministic; seed rows are audited but not copied.","Validation is a stratified stable-hash split; category-specific semantic checks run in the validator.","The final multitask file concatenates 94 cleaned research training rows, 215 identity/control training rows, and 500 distinct judged-correct trace-derived QA rows; optimizer sampling should still follow recommended_mix.json."],"unresolved_concerns":["Human review should spot-check nuanced capability, injection, and trace-derived answers before training."]}
