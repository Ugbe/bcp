...
try:
...
except ImportError as exc:  # tevatron is an optional, heavy dependency

    def _missing(name, reason):
        class _Unavailable:
            @classmethod
            def parse_args(cls, parser):
                raise ImportError(f"'{name}' searcher unavailable: {reason}")
    ...
    FaissSearcher = _missing("faiss", exc)
    ReasonIrSearcher = _missing("reasonir", exc)
