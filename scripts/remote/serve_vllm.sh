...
if [[ -z "${ADAPTER_REPO:-}" ]]; then
  echo "Starting base model ${BASE_MODEL} on ${VLLM_HOST:-0.0.0.0}:${VLLM_PORT}"
else
  require_env ADAPTER_REPO
  echo "Starting ${BASE_MODEL} with adapter ${ADAPTER_REPO} as ${SERVED_MODEL_NAME}"
fi
...
if [[ -n "${ADAPTER_REPO:-}" ]]; then
  vllm_args+=(
    ...
    --lora-modules "{\"name\":\"${SERVED_MODEL_NAME}\",\"path\":\"${ADAPTER_REPO}\",\"base_model_name\":\"${BASE_MODEL}\"}"
  )
else
